...
// Control Navegacion
    control= Ext.create('GeoExt.Action',{
        ...
        glyph:Global.setIcon('pan'),
      	//iconCls:'control-pan',
      	control: new OpenLayers.Control.Navigation({zoomBoxKeyMask:1}),
      	map:mapa,
      	tooltip: 'Navegacion',
        pressed: false
	});
...
// Control Zoom a Cuadricula
    control= Ext.create('GeoExt.Action',{
    	text:'',
    	iconCls:'zoomin',
    	control: new OpenLayers.Control.ZoomIn({zoomBoxKeyMask:0}),
    	map:mapa,
	...
	var style=new OpenLayers.Style();
		  style.addRules([new OpenLayers.Rule({symbolizer:sketchStyle})]);
	...
	var dist=  new OpenLayers.Control.Measure(OpenLayers.Handler.Path,{
              	...
              	eventListeners:{
                      ...
                      if(c<2){
            			    ...
            			    var l=Ext.Msg.show({title:'Medir Distancia', msg:  'Medicion de linea<br>Segmento: '+mparcial+' '+event.units+'</br>Total: '+Number(event.measure).toFixed(3) + ' ' + event.units, padding:'0 2 0 2',height:40,closable: true, modal:false/*, buttons:Ext.Msg.OK*/});
             				  l.setXY([1100,100]);
             			  }
          			}
        });
...
var buscaStore=Ext.create('Ext.data.Store',{// definir las opcciones para el combobox
		...
		data:[
			{elemento:'terreno',nombre:'Terreno'},
			{elemento:'manzana',nombre:'Manzana'}
		],
		autoLoad: false
});
